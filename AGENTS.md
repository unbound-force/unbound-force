...
│   └── sandbox.go                   # Sandbox command group (init, start, stop, attach, extract, status)
├── cmd/mutimind/
...
│       └── devcontainer/            # Devcontainer template (uf sandbox init)
...
│   ├── devpod.go                    # DevPodBackend: DevPod CDE workspace provisioning
...
- Go 1.24+ + `github.com/spf13/cobra` (CLI), `gopkg.in/yaml.v3` (config), `os/exec` (subprocess) (029-sandbox-cde-lifecycle)
- Podman named volumes for persistent state, DevPod for CDE workspaces (029-sandbox-cde-lifecycle)
...
- opsx/cde-sandbox-enhancement: Replaced Eclipse Che CDE backend with DevPod. Deleted `internal/sandbox/che.go` and all Che-specific code (`BackendChe` constant, `resolveCheURL()`, `CheURL` field, `CheConfig` struct, Che config fields, Che doctor checks). Added `DevPodBackend` in `internal/sandbox/devpod.go` implementing `Backend` interface — delegates workspace lifecycle to `devpod` CLI (`devpod up`, `devpod stop`, `devpod delete`, `devpod status`). DevPod requires Podman as provider and `devcontainer.json` for workspace configuration. Added `parseDevPodVersion()` enforcing DevPod >= 0.5.0. Auto-detection prefers DevPod when `devpod` is in PATH AND `.devcontainer/devcontainer.json` exists, falls back to Podman otherwise. `--backend che` returns migration error: "che backend removed, use --backend devpod instead". Wired `autoStartGateway()` into `Create()` and persistent `Start()` paths — gateway proxy now auto-starts for persistent workspaces (both Podman and DevPod). Gateway env vars injected via `--workspace-env` for DevPod and `buildPersistentRunArgs()` for Podman. Added `GatewayPort` and `GatewayActive` fields to `Options` struct. Created `uf sandbox init` subcommand that scaffolds `.devcontainer/devcontainer.json` from embedded template with `--image`, `--demo-ports`, and `--force` flags. Added DevPod doctor check group (binary presence, devcontainer config existence) gated on `isDevPodDetected()`. Updated CLI help text and flag descriptions. Modified files: `internal/sandbox/backend.go`, `internal/sandbox/devpod.go` (new), `internal/sandbox/sandbox.go`, `internal/sandbox/podman.go`, `internal/sandbox/workspace.go`, `internal/sandbox/config.go`, `internal/sandbox/detect.go`, `internal/sandbox/sandbox_test.go`, `internal/config/config.go`, `internal/config/config_test.go`, `internal/doctor/checks.go`, `internal/doctor/doctor_test.go`, `internal/scaffold/scaffold_test.go`, `internal/scaffold/assets/devcontainer/devcontainer.json` (new), `cmd/unbound-force/sandbox.go`, `cmd/unbound-force/sandbox_test.go`. Deleted: `internal/sandbox/che.go`. No new dependencies. 9 task groups completed.
