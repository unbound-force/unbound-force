...
pre-existing CI failures and post verdict-aligned
GitHub reviews (APPROVE / REQUEST CHANGES / COMMENT)
with human confirmation. Requires `gh` CLI.

Key capabilities:
- **PR walkthrough**: file-by-file change summary with
  path-based focus classification
- **Issue linking**: validates PR against linked GitHub
  issue acceptance criteria (`Fixes #N`)
- **Suggestion blocks**: one-click fixable code
  suggestions in GitHub's native format
- **Verdict-aligned posting**: posts review as APPROVE,
  REQUEST CHANGES, or COMMENT (affects merge
  eligibility). AI reviews are labeled as AI-generated.
- **Path-based focus**: built-in heuristics apply
  targeted review (security for API handlers, test
  quality for test files, etc.)
