...
│   ├── ollama_proxy.go              # Ollama proxy command group (start, stop, status)
...
├── internal/auth/                    # Token management for cloud providers (Vertex AI, Bedrock)
│   ├── token.go                     # TokenManager: background refresh, proactive refresh, atomic invalidation
│   ├── vertex.go                    # RefreshVertexToken (gcloud token acquisition)
│   ├── bedrock.go                   # RefreshBedrockCredentials, ParseEnvExport, ParseAWSCredentialsJSON
│   ├── token_test.go               # TokenManager lifecycle tests
│   ├── vertex_test.go              # Vertex token refresh tests
│   └── bedrock_test.go             # Bedrock credential refresh tests
├── internal/pidfile/                 # PID file management (extracted from gateway)
│   ├── pidfile.go                   # WritePID, ReadPID, IsAlive, CleanupStale, RemovePID, PIDInfo
...
│   ├── refresh.go                   # SigV4 signing (token refresh extracted to internal/auth/)
│   ├── pid.go                       # PID file delegation (delegates to internal/pidfile/)
│   ├── sse.go                       # SSE response filtering (Vertex event stripping)
│   └── gateway_test.go              # Tests for all gateway functions
├── internal/ollamaproxy/             # Ollama-compatible proxy for cloud embeddings (GPU-less workflow)
│   ├── proxy.go                     # Core proxy: Options, Start, Stop, Status, health endpoint, model mapping
│   ├── embed.go                     # /api/embed handler (Vertex AI text-embedding translation)
│   ├── generate.go                  # /api/generate handler (gateway-backed text generation)
│   ├── tags.go                      # /api/tags handler (synthetic model catalog)
│   └── proxy_test.go               # Tests for all proxy functions
...
- opsx/ollama-proxy: Added `uf ollama-proxy` command with 3 subcommands (`start`, `stop`, `status`) for Ollama-compatible proxy that translates embedding and generation requests to cloud providers (Vertex AI via gateway), enabling GPU-less developers to use Dewey's semantic search without local Ollama. Extracted `internal/pidfile/` package from `internal/gateway/pid.go` (`WritePID`, `ReadPID`, `IsAlive`, `CleanupStale`, `RemovePID`, `PIDInfo`, `signalZero`) — shared by both gateway and ollama-proxy. Extracted `internal/auth/` package from `internal/gateway/refresh.go` and `internal/gateway/provider.go` — `TokenManager` struct with background refresh, proactive refresh with `TryLock()` deduplication, atomic invalidation on failure; `RefreshVertexToken`, `RefreshBedrockCredentials`, `ParseEnvExport`, `ParseAWSCredentialsJSON`. Created `internal/ollamaproxy/` package (5 source files: `proxy.go`, `embed.go`, `generate.go`, `tags.go`, `proxy_test.go`) following the established `Options`/injectable-function pattern. Three endpoints: `POST /api/embed` (Vertex AI text-embedding translation with batch support), `POST /api/generate` (gateway-backed text generation via Anthropic Messages API), `GET /api/tags` (synthetic model catalog). Model name mapping table (`defaultModelMap`) with `granite-embedding:30m`, `granite-embedding-small-english-r2`, `llama3.2:3b`. Security hardening: SSRF prevention via loopback-only gateway URL validation (`validateGatewayURL`), Bearer token redaction in error responses (`redactToken`), model name validation rejecting path traversal (`validateModelName`), 10MB max request body size. Health endpoint at `GET /health` returns JSON with `service: "uf-ollama-proxy"` (distinguishes from real Ollama). Background mode via re-exec with `_UF_OLLAMA_PROXY_CHILD` sentinel. PID file at `.uf/ollama-proxy.pid`. Config integration: `OllamaProxyConfig` struct in `internal/config/` with `Port`, `EmbedModel`, `GatewayURL` fields, env var overrides (`UF_OLLAMA_PROXY_PORT`, `UF_OLLAMA_PROXY_EMBED_MODEL`, `UF_OLLAMA_PROXY_GATEWAY_URL`), config template section. Created `cmd/unbound-force/ollama_proxy.go` with Cobra command registration (`--port`, `--embed-model`, `--gateway-url`, `--detach` flags). Added 69 new test functions across 5 test files (29 proxy, 11 token manager, 5 vertex, 12 bedrock, 12 pidfile). Updated gateway to use extracted `pidfile` and `auth` packages. Modified files: `internal/ollamaproxy/proxy.go`, `internal/ollamaproxy/embed.go`, `internal/ollamaproxy/generate.go`, `internal/ollamaproxy/tags.go`, `internal/ollamaproxy/proxy_test.go`, `internal/auth/token.go`, `internal/auth/vertex.go`, `internal/auth/bedrock.go`, `internal/auth/token_test.go`, `internal/auth/vertex_test.go`, `internal/auth/bedrock_test.go`, `internal/pidfile/pidfile.go`, `internal/pidfile/signal_unix.go`, `internal/pidfile/pidfile_test.go`, `internal/gateway/pid.go`, `internal/gateway/refresh.go`, `internal/gateway/provider.go`, `internal/gateway/gateway.go`, `internal/gateway/gateway_test.go`, `internal/config/config.go`, `internal/config/config_test.go`, `cmd/unbound-force/ollama_proxy.go`, `cmd/unbound-force/main.go`. No new dependencies. 11 task groups and 107 tasks completed.
