...
- feat/opkg-delegate: Added OpenPackage (`opkg`) delegation to `uf init` and `uf setup` for multi-harness support (OpenCode, Cursor, Claude Code). `uf init` now embeds two opkg package source trees under `.openpackage/packages/` (review-council + workflows) and delegates to `opkg install` when `opkg` is on PATH — using absolute paths to the locally deployed packages. New `--platforms` flag on `uf init` passes comma-separated harness names to `opkg install --platforms`. Post-walk overlay model: embedded assets are always written as baseline; `opkg` routes content to harness-specific directories (`.opencode/`, `.cursor/`, `.claude/`) on top. Graceful fallback when `opkg` absent. `uf setup` step 15 installs `opkg` via Homebrew. New `Options.ExecCmdInDir`, `Options.SkipOpenPackage`, `Options.OpenPackagePlatforms` fields. Removed `assetDelegatedToOpenPackage()` dead helper. Added 4 new scaffold tests (`TestRun_OpenPackageRunsAfterScaffold` extended, `TestRun_OpenPackagePlatforms`, `TestRun_OpenPackageFailureReportsPackageName`, `TestRun_OpenPackageDivisorOnly`) and 5 new setup tests (`TestInstallOpkg_*`). Modified files: `internal/scaffold/scaffold.go`, `internal/scaffold/scaffold_test.go`, `internal/scaffold/assets/openpackage/` (35 new embedded files), `internal/setup/setup.go`, `internal/setup/setup_test.go`, `cmd/unbound-force/main.go`, `cmd/unbound-force/main_test.go`, `openspec/changes/opkg-delegate/`. All tasks completed.
