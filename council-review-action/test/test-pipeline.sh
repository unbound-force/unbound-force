...
for tool in python3 jq; do
  command -v "${tool}" >/dev/null 2>&1 || {
    echo "FATAL: ${tool} required but not found" >&2
    exit 1
  }
done
...
assert_contains "no fix loops" "Do NOT iterate fix loops" "${PROMPT}"
...
# NOTE: Reads prompt file generated by Test 14 (intentional —
# the schema section is static and independent of PR title).
...
# ── Test 27: extract-review-json.py — MAX_PARSE_ATTEMPTS ─────
echo "Test 27: extract-review-json.py — parse attempt limit"

ATTEMPT_DIR=$(mktemp -d)
# Generate input with many '{' characters that form valid JSON
# objects but lack the required keys, forcing many parse attempts.
python3 -c "print('{\"a\":1} ' * 600)" > "${ATTEMPT_DIR}/review_text.txt"
if (cd "${ATTEMPT_DIR}" && python3 "${SCRIPT_DIR}/extract-review-json.py" > /dev/null 2>&1); then
  echo "  FAIL: should reject input exceeding parse attempts"
  FAIL=$((FAIL + 1))
else
  echo "  PASS: rejects input exceeding parse attempts"
  PASS=$((PASS + 1))
fi
rm -rf "${ATTEMPT_DIR}"

# ── Test 28: Noise filter — multiple pattern categories ──────
echo "Test 28: Noise filter covers lock, vendor, generated, fixtures"

cat > "${WORK_DIR}/noise-multi.patch" << 'DIFF'
...
@@ -0,0 +1,1 @@
+generated
diff --git a/testdata/golden.txt b/testdata/golden.txt
new file mode 100644
--- /dev/null
...
(cd "${WORK_DIR}" && DIFF_PATH=noise-multi.patch bash "${SCRIPT_DIR}/prepare-diff.sh")

FILTERED=$(cat "${WORK_DIR}/pr-diff-filtered.patch")
assert_not_contains "package-lock.json excluded" "package-lock.json" "${FILTERED}"
assert_not_contains "vendor/ excluded" "vendor/lib.go" "${FILTERED}"
assert_not_contains "api.pb.go excluded" "api.pb.go" "${FILTERED}"
assert_not_contains "testdata/ excluded" "testdata/golden.txt" "${FILTERED}"
assert_contains "real.go kept" "real.go" "${FILTERED}"

# ── Test 29: filter-diff-lines.py — empty inline_comments ────
echo "Test 29: filter-diff-lines.py — empty comments array"

EMPTY_DIR=$(mktemp -d)
cat > "${EMPTY_DIR}/review_output.json" << 'JSON'
{"summary": "Clean code.", "inline_comments": []}
JSON
cat > "${EMPTY_DIR}/pr-diff-filtered.patch" << 'DIFF'
diff --git a/a.go b/a.go
new file mode 100644
--- /dev/null
+++ b/a.go
...
+package main
DIFF
python3 "${SCRIPT_DIR}/filter-diff-lines.py" \
  "${EMPTY_DIR}/pr-diff-filtered.patch" \
  "${EMPTY_DIR}/review_output.json" \
  > "${EMPTY_DIR}/filtered_output.json" 2>/dev/null
COUNT=$(jq '.inline_comments | length' "${EMPTY_DIR}/filtered_output.json")
SUMMARY=$(jq -r '.summary' "${EMPTY_DIR}/filtered_output.json")
assert_eq "empty comments pass through" "0" "${COUNT}"
assert_eq "summary unchanged" "Clean code." "${SUMMARY}"
rm -rf "${EMPTY_DIR}"

# ── Test 30: prepare-diff.sh — rename diff headers ───────────
echo "Test 30: prepare-diff.sh — rename headers pass through"

cat > "${WORK_DIR}/rename.patch" << 'DIFF'
...
(cd "${WORK_DIR}" && DIFF_PATH=rename.patch bash "${SCRIPT_DIR}/prepare-diff.sh")

ANNOTATED=$(cat "${WORK_DIR}/pr-diff-annotated.patch")
assert_contains "rename from preserved" "rename from old.py" "${ANNOTATED}"
assert_contains "rename to preserved" "rename to new.py" "${ANNOTATED}"
assert_contains "keep.go annotated" "[L1] +package main" "${ANNOTATED}"

# ── Test 31: build-prompt.sh — special chars in PR title ─────
echo "Test 31: build-prompt.sh — special characters in title"

mkdir -p "${WORK_DIR}/prompt-special"
jq -n '{title: "feat: fix $(whoami) \"injection\" {test} `cmd`"}' \
  > "${WORK_DIR}/prompt-special/meta.json"

(cd "${WORK_DIR}/prompt-special" && \
  META_PATH=meta.json \
  bash "${SCRIPT_DIR}/build-prompt.sh")
