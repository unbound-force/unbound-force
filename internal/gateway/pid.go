...
// PIDInfo represents the contents of the PID file at
// .uf/gateway.pid. The file format is plain text with
// the PID on line 1 and key=value metadata on subsequent
// lines (per data-model.md).
type PIDInfo struct {
	// PID is the process ID of the gateway.
	PID int

	// Port is the local port the gateway listens on.
	Port int
	...
	// Started is the time the gateway was started.
	Started time.Time
}

// WritePID writes the PID file atomically (write to temp
// file, then rename) to prevent partial reads. The file
// is written to the .uf/ directory under the project root.
//
